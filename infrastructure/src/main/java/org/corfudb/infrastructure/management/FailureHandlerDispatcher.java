...
public class FailureHandlerDispatcher {
    ...
    public boolean recoverCluster(Layout recoveryLayout, CorfuRuntime corfuRuntime) {
        ...
        try {
            return runLayoutReconfiguration(recoveryLayout, recoveryLayout, corfuRuntime, true);
        } catch (Exception e) {
     ...
     *
     * @param sealLayout   Layout which needs to be sealed.
     ...
     */
    private boolean runLayoutReconfiguration(Layout sealLayout, Layout newLayout,
                                             CorfuRuntime corfuRuntime, final boolean recovery)
            throws Exception {

        // Seals and increments the epoch.
        sealLayout.setRuntime(corfuRuntime);
        sealEpoch(sealLayout);
        ...
        reconfigureServers(corfuRuntime, sealLayout, newLayout, recovery);
