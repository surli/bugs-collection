...
public class VersionLockedObject<T> {
    ...
    public boolean needUndoRecordUnsafe() {
        ...
        // make sure we have at most enough undo entries
        // to go between this version and the oldest version
        while (undoLog.size() > version - oldestVersion + 1) {
            undoLog.pollLast();
        }
    ...
    public Object applyUpdateUnsafe(SMREntry entry, boolean isOptimistic) {
        ...
        try {
            ...
            if (undoRecordTarget != null) {
                ...
                // undo logs, and mark that the object is
